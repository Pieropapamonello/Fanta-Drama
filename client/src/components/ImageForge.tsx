...
export default function ImageForge({ kind, imageUrl, onChange }: { kind: Kind; imageUrl?: string; onChange: (url: string) => void }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setMessage(code === 'openai_image_generation_not_configured' ? 'La generazione IA deve ancora essere attivata dall’amministratore.' : code === 'daily_generation_limit_reached' ? 'Hai raggiunto il limite giornaliero di immagini IA.' : 'Non riesco a generare l’immagine. Modifica la descrizione e riprova.')
