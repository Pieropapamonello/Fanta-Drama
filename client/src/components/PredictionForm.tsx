...
export default function PredictionForm({ eventId, phase, acquisitionMode = 'AUCTION', onSaved }: Props) {
  ...
  const vote = async (claimItem: any, decision: 'CONFIRM' | 'DENY') => { setWorking(`vote-${claimItem.id}`); setMessage(''); try { const response = await api.post(`/claims/${claimItem.id}/vote`, { vote: decision }); const result = response.data; setMessage(result.alreadyVoted ? 'Avevi già registrato questa decisione.' : result.status === 'CONFIRMED' ? 'Carta confermata: punti assegnati e crew avvisata.' : result.status === 'DENIED' ? 'Carta contestata: la crew è stata avvisata.' : `${decision === 'CONFIRM' ? 'Conferma' : 'Contestazione'} registrata: ${result.confirms} conferme e ${result.denies} contestazioni.`); await load(); onSaved?.() } catch (error: any) { const code = error.response?.data?.error; setMessage(code === 'claimant_cannot_vote' ? 'Non puoi approvare la tua stessa carta.' : code === 'claim_already_resolved' ? 'Questa carta è già stata risolta.' : 'Non riesco a registrare la decisione.') } finally { setWorking('') } }
