...
export default function Dashboard() {
  ...
  return <div><RegisterPasskeyButton />
    ...
    <section className="dashboard-stats">{[[Trophy, `${stats?.credits ?? 0}`, 'Crediti disponibili', '/events'], [Users, `${stats?.groups ?? 0}`, 'Gruppi attivi', '/groups'], [Layers, `${stats?.cards ?? 0}`, 'Carte create', '/cards']].map(([Icon, value, label, to]: any) => <Link key={label} to={to} aria-label={`Apri ${label.toLowerCase()}`}><Icon size={18} /><strong>{value}</strong><span>{label}</span></Link>)}</section>
