...
const schema = z.object({ title: z.string().trim().min(3), description: z.string().trim().min(12), basePoints: z.number().min(0).max(100).optional(), imageUrl: z.string().url(), imageStoragePath: z.string().optional() })
...
export default function CreateCard() {
  ...
  return <form onSubmit={handleSubmit(onSubmit)} className="max-w-md"><p className="eyebrow">Collezione condivisa</p><h2 className="text-2xl mb-4">Crea una carta unica</h2><p className="page-lead">La tua carta entrerà nel catalogo e potrà essere aggiunta al mazzo di tutti.</p><label className="block mb-2">Titolo</label><input {...register('title')} className="input" placeholder="Es. Brindisi proibito" />{errors.title && <p className="profile-error">Inserisci almeno 3 caratteri.</p>}<label className="block mt-4 mb-2">Effetto / descrizione unica</label><textarea {...register('description')} className="input profile-textarea" placeholder="Es. Indovina chi farà il brindisi più imbarazzante della serata." />{errors.description && <p className="profile-error">Descrivi la carta con almeno 12 caratteri.</p>}<label className="block mt-4 mb-2">Punti base</label><input type="number" defaultValue={10} {...register('basePoints', { valueAsNumber: true })} className="input" /><ImageForge kind="CARD" imageUrl={watch('imageUrl')} onChange={(url, storagePath) => { setValue('imageUrl', url, { shouldValidate: true }); setValue('imageStoragePath', storagePath) }} />{errors.imageUrl && <p className="profile-error">Genera l’immagine unica della carta prima di pubblicarla.</p>}{error && <p className="profile-error">{error === 'card_title_already_exists' ? 'Esiste già una carta con questo titolo.' : error === 'card_description_already_exists' ? 'Esiste già una carta con questa descrizione.' : error === 'card_image_already_exists' ? 'Questa immagine appartiene già a un’altra carta.' : error}</p>}<button className="btn mt-4" disabled={isSubmitting}>{isSubmitting ? 'Pubblico…' : 'Pubblica carta per tutti'}</button></form>
}
