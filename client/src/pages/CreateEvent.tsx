...
export default function CreateEvent() {
  ...
  const onSubmit = async (data: any) => {
    try {
      setError(null)
      await api.post('/events', { ...data, startsAt: new Date(data.startsAt).toISOString(), endsAt: new Date(data.endsAt).toISOString() })
      navigate('/events')
    } catch (err: any) { setError(err.response?.data?.error || 'Non riesco a creare l’evento.') }
  }
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md">
      ...
      <label className="block mt-4 mb-2">Inizio</label>
      <input type="datetime-local" {...register('startsAt')} className="input" />
      <label className="block mt-4 mb-2">Fine</label>
